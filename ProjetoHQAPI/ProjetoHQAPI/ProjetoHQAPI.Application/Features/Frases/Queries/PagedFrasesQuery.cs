...
{
    public partial class PagedFrasesQuery : IRequest<PagedDataTableResponse<IEnumerable<Entity>>>
    {
    ...
    public class PagedFrasesQueryHandler : IRequestHandler<PagedFrasesQuery, PagedDataTableResponse<IEnumerable<Entity>>>
    {
        private readonly IFraseRepositoryAsync _frasesRepositoryAsync;
        private readonly IMapper _mapper;
        private readonly IModelHelper _modelHelper;

        public PagedFrasesQueryHandler(IFraseRepositoryAsync fraseRepository, IMapper mapper, IModelHelper modelHelper)
        {
            _frasesRepositoryAsync = fraseRepository;
            _mapper = mapper;
            _modelHelper = modelHelper;
        }

        public async Task<PagedDataTableResponse<IEnumerable<Entity>>> Handle(PagedFrasesQuery request, CancellationToken cancellationToken)
        {
            var validFilter = new GetFrasesQuery();
            ...
            {
                case 0:
                    validFilter.OrderBy = colOrder.Dir == "asc" ? "NomeHQ" : "NomeHQ DESC";
                    break;
            }
            ...
            if (!string.IsNullOrEmpty(request.Search.Value))
            {
                ...
                validFilter.NomeHQ = request.Search.Value;
            }
            ...
            var entityPositions = await _frasesRepositoryAsync.GetPagedFrasesReponseAsync(validFilter);
